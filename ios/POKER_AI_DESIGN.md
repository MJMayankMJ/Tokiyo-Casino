...
**Phase 2 — profiles + difficulty UI (DELIVERED):** `AIProfile`/`Difficulty`, the settings UI, persistence, and multiplayer host plumbing. `.solverInspired` preset (not "GTO"). Player can dial difficulty and style up or down. Shipped: `Difficulty.swift` (tiers + `PokerAIStyle` + `PokerAIConfig` + `PokerAIConfigStore`), `Difficulty.apply(to:)` clamping budget/mistake to the tier while preserving style, `PokerAIConfig.resolvedProfiles(aiSeatCount:)` (pinned-style or per-tier auto-mix), `Player.aiProfile`/`resolvedProfile`, `GameManager.aiConfig` + `applyAIConfigToAISeats()`, a Difficulty segmented control on the solo setup menu persisted under the `pokerAIConfig` `UserDefaults` key, and the multiplayer host reading the same config. `DifficultyTests` covers the tier table, apply clamping, auto-mix/pinned resolution, and config round-trip. The `exploitation` flag is stored per tier but inert until Phase 3.

**Phase 3 — adaptation (DELIVERED):** `HandHistoryTracker`/`ActionLog`, `OpponentModel`, `Exploit` layer. Shipped: `HandHistoryTracker` (a passive observer `GameManager` drives via `handStarted` / `streetBegan` / `recordAction` / `handEnded`, folding events into per-seat `OpponentStats`); pre-action context (`callAmount`, `raisedBet`) snapshotted in `processPlayerAction` *before* `executeAction` mutates state, per §6.1; `OpponentStats` opportunity counts for VPIP / PFR / postflop AF / fold-to-cbet / fold-to-3bet / WTSD; `OpponentModel` blending each stat with an `AIProfile.populationBaseline`-derived prior, ramping to full trust by ~30 opportunities; `Exploit.adjusted(profile:vs:intensity:)` tilting against calling-station / nit / maniac archetypes, gated to Hard (`.on`) and Expert (`.aggressive`) via `Difficulty.exploitation` with an adaptation ramp so it stays inert until ~30 observed hands; `GameManager.processAITurn` applies the exploit against the most-observed active human. The model resets per session (the tracker lives on `GameManager`). `OpponentModelingTests` covers the tracker's event folding (VPIP/PFR, fold-to-3bet, fold-to-cbet incl. donk-bet exclusion, AF incl. all-in-call classification, WTSD), the model's sample-size blending, and the exploit adjustments (off/no-signal identity, station/nit/maniac tilts, clamping).
